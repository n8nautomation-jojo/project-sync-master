...
CREATE INDEX idx_rate_limits_org ON public.webhook_rate_limits(organization_id, window_start);
...
CREATE POLICY "Admins can delete whatsapp connections" ON public.whatsapp_connections FOR DELETE TO authenticated USING (public.has_organization_role(auth.uid(), organization_id, ARRAY['owner', 'admin']::public.app_role[]));
...
CREATE POLICY "Members can insert whatsapp messages" ON public.whatsapp_messages FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "Service role can insert notifications" ON public.notifications FOR INSERT TO service_role WITH CHECK (auth.role() = 'service_role' AND user_id IS NOT NULL AND title <> '' AND message <> '');
...
-- Audit Logs
CREATE POLICY "Users can view their organization audit logs" ON public.audit_logs FOR SELECT TO authenticated USING (organization_id IN (SELECT get_user_organization_ids(auth.uid())));
CREATE POLICY "System can insert audit logs" ON public.audit_logs FOR INSERT TO authenticated, service_role WITH CHECK ((auth.uid() IS NOT NULL OR auth.role() = 'service_role') AND table_name <> '' AND action IN ('create', 'update', 'delete'));

-- Failed Jobs
CREATE POLICY "Admins can view failed jobs" ON public.failed_jobs FOR SELECT TO authenticated USING (organization_id IN (SELECT get_user_organization_ids(auth.uid())));
CREATE POLICY "Service role insert failed jobs" ON public.failed_jobs FOR INSERT TO authenticated WITH CHECK (organization_id IN (SELECT get_user_organization_ids(auth.uid())));
CREATE POLICY "Service role update failed jobs" ON public.failed_jobs FOR UPDATE TO authenticated USING (organization_id IN (SELECT get_user_organization_ids(auth.uid())));

-- System Logs
CREATE POLICY "Admins can view system logs" ON public.system_logs FOR SELECT TO authenticated USING (organization_id IN (SELECT get_user_organization_ids(auth.uid())));
CREATE POLICY "Members can insert system logs" ON public.system_logs FOR INSERT TO authenticated WITH CHECK (organization_id IN (SELECT get_user_organization_ids(auth.uid())));

-- Webhook Rate Limits
CREATE POLICY "Service role can view webhook rate limits" ON public.webhook_rate_limits FOR SELECT TO service_role USING (auth.role() = 'service_role');
CREATE POLICY "Service role can insert webhook rate limits" ON public.webhook_rate_limits FOR INSERT TO service_role WITH CHECK (auth.role() = 'service_role');
CREATE POLICY "Service role can update webhook rate limits" ON public.webhook_rate_limits FOR UPDATE TO service_role USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');

-- ============ REALTIME ============
ALTER PUBLICATION supabase_realtime ADD TABLE public.notifications;
ALTER PUBLICATION supabase_realtime ADD TABLE public.transfers;
ALTER PUBLICATION supabase_realtime ADD TABLE public.whatsapp_messages;
ALTER PUBLICATION supabase_realtime ADD TABLE public.failed_jobs;
